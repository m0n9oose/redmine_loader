class LoaderController < ApplicationController
  ...
  def create
    ...
    begin
      ...
      if tasks_to_import.size <= tasks_per_time
        uid_to_issue_id, outlinenumber_to_issue_id = Loader.import_tasks(tasks_to_import, @project.id, user, nil, update_existing)
        Loader.map_subtasks_and_parents(issues_info, @project.id, nil, uid_to_issue_id, outlinenumber_to_issue_id)
        Loader.map_versions_and_relations(milestones, issues, @project.id, nil, uid_to_issue_id)
  ...
  def determine_nesting(issues, versions_count=0)
    nested_issues = []
  ...
  def write_task(xml, struct, id)
    ...
    xml.Task {
      ...
      xml.ConstraintType "0"
      xml.ConstraintDate struct.issue.next_working_date(start_date).to_time.to_s(:ms_xml)
